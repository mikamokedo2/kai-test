...
const {
  contract: { kaiAddressContract, abi, bscAddressContract },
} = require("./const");
const app = express();
...
const KAI_RPC_ENDPOINT =
  process.env.PROVIDERKAI || "https://dev.kardiachain.io";

const BSC_RPC_ENDPOINT =
  process.env.PROVIDERKAI || "https://data-seed-prebsc-1-s1.binance.org:8545";

const web3kai = new Web3(new Web3.providers.HttpProvider(KAI_RPC_ENDPOINT));
const web3bsc = new Web3(new Web3.providers.HttpProvider(BSC_RPC_ENDPOINT));
const adminWalletKai = web3kai.eth.accounts.privateKeyToAccount(privateKey);
web3kai.eth.accounts.wallet.add(adminWalletKai);
const shopdiContractKai = new web3kai.eth.Contract(abi, kaiAddressContract);
const adminWalletBsc = web3bsc.eth.accounts.privateKeyToAccount(privateKey);
web3bsc.eth.accounts.wallet.add(adminWalletBsc);
const shopdiContractBsc = new web3bsc.eth.Contract(abi, bscAddressContract);
...
app.get("/adminWalletAddress", async (req, res) => {
  const net = req.query.type;
  return res
    .status(200)
    .send({
      success: true,
      data: {
        wallet: net === "kai" ? adminWalletKai.address : adminWalletBsc.address,
        rate: rateConvert,
        contractAddress:
          net === "kai" ? kaiAddressContract : bscAddressContract,
      },
    });
});
...
app.post("/order", async function (req, res) {
  const { user, amount, emailorphone, value, txt, captcha, netWork } = req.body;
  if (
    !user ||
    !amount ||
    !emailorphone ||
  ...
  try {
    ...
    if (netWork === "kai") {
      await shopdiContractKai.methods
        .transferFrom(user, adminWalletKai.address, amount)
        .send({
          from: adminWalletKai.address,
          gas: "1000000",
        });
    } else {
      await shopdiContractBsc.methods
        .transferFrom(user, adminWalletBsc.address, amount)
        .send({
          from: adminWalletBsc.address,
          gas: "1000000",
        });
    }
